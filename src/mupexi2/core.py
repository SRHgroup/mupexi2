...
def open_text_maybe_gzip(path):
    return _open_text_maybe_gzip(path)
...
def parse_info_field(info_field):
    return vcf_utils.parse_info_field(info_field)


def infer_source_set_from_info(info_field):
    return vcf_utils.infer_source_set_from_info(info_field)
...
def parse_genotype_state(gt_value, ps_value):
    return vcf_utils.parse_genotype_state(gt_value, ps_value)


def should_apply_context_variant(primary_state, context_state):
    return vcf_utils.should_apply_context_variant(primary_state, context_state)
...
def resolve_sample_indices(header, tumor_sample=None, normal_sample=None):
    try:
        return vcf_utils.resolve_sample_indices(header, tumor_sample, normal_sample)
...
def _run_single_k_task(p_length, vep_info, proteome_reference, genome_reference, reference_peptides,
                       reference_peptide_file_names, fasta_file_name, peptide_match, tmp_dir, webserver,
                       print_mismatch, num_mismatches, superpeptides):
    ...
    return {
        ...
        'pepmatch_file_name': None if p_pepmatch_file_names.get(p_length, None) is None else
        (p_pepmatch_file_names[p_length] if isinstance(p_pepmatch_file_names[p_length], str)
         else p_pepmatch_file_names[p_length].name),
    }
